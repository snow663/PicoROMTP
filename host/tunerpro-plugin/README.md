...
> If `host/tunerpro-plugin/TunerProPluginSDK.zip` is present, CMake will extract and use the
> official TunerPro Plug‑in SDK automatically. You can still point at another SDK copy via
> `-DTUNERPRO_SDK_ROOT` if desired.

## Building

1. Install Visual Studio with the Desktop development with C++ workload.
2. Open a *x64 Native Tools* command prompt (PowerShell is fine).
3. From the repo root, run the helper script (this extracts the bundled SDK automatically):

```powershell
pwsh -ExecutionPolicy Bypass -File host/tunerpro-plugin/build.ps1
```

You can override the build config or generator if needed, e.g. `-Config Debug` or
`-Generator "Visual Studio 17 2022"`. Add `-Clean` to remove the existing `build/` folder
before configuring.

The build will produce `PicoROMTunerPro.dll` in `host/tunerpro-plugin/build/Release/`.
Copy this into your `TunerPro\Plugins` directory.
